...
def newcatGPT(text):
    completion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are an assistant meant to turn the following not-cute block of text into a cute silly uwu cat version of the same text.  Use an extreme amount of uwu cute speak.",
            },
...
def berlockGPT(text):
    completion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are Berlock, an occult being capable of generating proverbs based on topics. Make them as weird, nonsensical, and mysterious as possible. Keep them short and extremely sharp! Humans need not understand them.",
            },
            {"role": "user", "content": f"'''{text}'''"},
        ],
        temperature=1.75,
...
def grubGPT(text):
    completion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are an assistant that converts provided text into the language of grubworld. Grubworld is a world where everything is replaced with grubs. All of society and every aspect of life is structured around grubs. Feel free to be creative in order to grubify the message as much as possible.",
            },
            {"role": "user", "content": f"'''{text}'''"},
        ],
        max_tokens=400,
...
def infographic_image(quote):
    ...
    image_editable.text(
        (640, 640), title_text, (11, 11, 11), font=title_font, anchor="mm"
    )
